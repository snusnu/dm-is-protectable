...
Guards are the part that actually enforce the security permissions.
...
** if a @Hash@ is used, _the only values that are recognized_, are @true@, @false@, a @Symbol@, a @lambda@, or a @Proc@.
...
_before hooks_ are registered on these methods that will raise 
